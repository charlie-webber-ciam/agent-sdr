...
export default function ParentCompanyProgressPage({
  params,
}: {
  ...
  const checkJobActive = async (jobStatus: string) => {
    if (jobStatus === 'processing') {
      try {
        const res = await fetch(`/api/parent-company/jobs/${jobId}/active`);
        if (res.ok) {
  ...
  const { loading } = useJobPolling<JobData>({
    url: `/api/parent-company/jobs/${jobId}`,
    ...
    return (
      <main className="min-h-screen p-8 max-w-7xl mx-auto">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <button
            onClick={() => router.push('/parent-company')}
            className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700"
          >
            Back to Parent Company Finder
          </button>
        </div>
      </main>
    );
  ...
  return (
    <main className="min-h-screen p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Parent Company Finder Job #{job.id}</p>
      </div>
      ...
      {isInterrupted && (
        <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-amber-800">Detection was interrupted</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
            <span>Detecting parent companies...</span>
        ...
        {isCompleted && (
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-green-900">Detection Complete!</span>
            </div>
        ...
        {isFailed && (
          <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-red-900">Detection Failed</span>
            </div>
            <p className="text-sm text-red-700">
              The parent company detection job encountered an error. Please try again.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div className="font-medium">
                {job.filters.unprocessedOnly ? 'Unprocessed Only' : 'All Accounts'}
              </div>
            </div>
            {job.filters.limit && (
              <div>
