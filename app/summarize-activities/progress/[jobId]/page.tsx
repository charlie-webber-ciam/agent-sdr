...
export default function ActivitySummarizationProgressPage({
  params,
}: {
  ...
  const checkJobActive = async (jobStatus: string) => {
    if (jobStatus === 'processing') {
      try {
        const res = await fetch(`/api/activity-summarization/jobs/${jobId}/active`);
        if (res.ok) {
  ...
  const { loading } = useJobPolling<JobData>({
    url: `/api/activity-summarization/jobs/${jobId}`,
    ...
    return (
      <main className="min-h-screen p-8 max-w-7xl mx-auto">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <button
            onClick={() => router.push('/summarize-activities')}
  ...
  return (
    <main className="min-h-screen p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">Activity Summarization Job #{job.id}</p>
      </div>
      ...
      {isInterrupted && (
        <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-amber-800">Summarization was interrupted</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between text-sm text-gray-600 mb-2">
            <span>Summarizing activities...</span>
        ...
        {isProcessing && !isInterrupted && currentAccount && (
          <div className="mt-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-indigo-900">Currently Processing</span>
            </div>
            <div className="text-lg font-bold text-indigo-900">
              {currentAccount.company_name}
            </div>
            {currentAccount.domain && (
              <div className="text-sm text-indigo-700">{currentAccount.domain}</div>
            )}
          </div>
        )}
        ...
        {isCompleted && (
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-green-900">Summarization Complete!</span>
            </div>
            <p className="text-sm text-green-700 mb-3">
              Successfully summarized activities for {job.processed_count} accounts.
        ...
        {isFailed && (
          <div className="mt-6 p-4 bg-red-50 border border-red-200 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-red-900">Summarization Failed</span>
            </div>
            <p className="text-sm text-red-700">
              The summarization job encountered an error. Please try again.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <div className="font-medium">
                {job.filters.unsummarizedOnly ? 'Unsummarized Only' : 'All Accounts'}
              </div>
            </div>
            {job.filters.limit && (
              <div>
